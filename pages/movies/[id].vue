  ...
  <div class="flex flex-col px-20 mt-10">
        <div class="grid grid-cols-7 gap-1">
            ...
            <div class="flex flex-col col-span-3">
                ...
                <div class="flex">
                    <div class="px-4 py-2 bg-gray-200 text-gray-800 rounded-full mr-2 mb-2" v-for="genre in data?.genres" :key="genre">{{ genre.name }}</div>
                </div>
...
const moviedId = computed(() => route.params.id);

const {data} = await useFetch<Movie>(`/api/movies/${moviedId.value}`)
